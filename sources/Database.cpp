...
void Database::initialize() {
	...
	string dropProcedureTableSQL = "DROP TABLE IF EXISTS Users";
	sqlite3_exec(dbConnection, dropProcedureTableSQL.c_str(), NULL, 0, &errorMessage);

	// create a procedure table
	string createProcedureTableSQL = "CREATE TABLE Users (userName VARCHAR(255) PRIMARY KEY);";
...
void Database::insertProcedure(string procedureName) {
	string insertProcedureSQL = "INSERT INTO Users ('userName') VALUES ('" + procedureName + "');";
...
void Database::getProcedures(vector<string>& results){
	...
	string getProceduresSQL = "SELECT * FROM Users;";
